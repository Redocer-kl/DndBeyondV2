...
class GenerateCharacterView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response({"error": "AI returned invalid JSON"}, status=400)

        AIConceptLog.objects.create(
            user=request.user,
            prompt=full_prompt,
            ai_response=char_data
        )

        builder = CharacterBuilder(request.user, char_data, user_concept)
        character = builder.create_character()

        return Response({"id": character.id, "name": character.name})